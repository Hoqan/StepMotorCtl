...
// clang-format off
static const unsigned char DangMap[] = {
/*--  文字:  加  --*/
...
0x49,0x28,0x49,0x24,0x6A,0x24,0x52,0x22,0x44,0x22,0x40,0x20,0x40,0xA0,0x40,0x40,
...
0x11,0x24,0x11,0xFC,0x1D,0x24,0xF1,0x24,0x51,0x24,0x11,0x24,0x11,0xFC,0x11,0x04,
                    ...
                    uint16_t DrawModel)  // 是否反色显示
{
  ...
  c = &DangMap[index * Font_Heig * Font_width / 8];
  ...
  for (i = 0; i < height; i++) // 遍历字体高度绘点
  {
    for (j = 0; j < width / 8; j++) // 遍历字体宽度绘点
    {
      for (k = 0; k < 8; k++)
      {
        bit = (*c << k) & 0x80;
        if (bit != DrawModel) // 每一个字节表示一个像素，进行描点显示
        {
          LCD_DrawPixel((Xpos + j * 8 + k), Ypos, DrawProp[ActiveLayer].TextColor);
        }
        else
        {
          LCD_DrawPixel((Xpos + j * 8 + k), Ypos, DrawProp[ActiveLayer].BackColor);
        }
      }
      c++;
    }
...
void LCD_DisplayStringCh(uint16_t x,          // 字符显示位置x
                         uint16_t y,          // 字符显示位置y
                             ...
                             uint16_t DrawModel)  // 是否反色显示
{
  ...
  while ((refcolumn < LCD_PIXEL_WIDTH) && ((*ptr != 0) & (((refcolumn + DrawProp[ActiveLayer].pFont->Width) & 0xFFFF) >= DrawProp[ActiveLayer].pFont->Width)))
  {
    ...
    else
    {
      ...
      Ascii = *ptr - 32;
      // 缩放字模数据
      LCD_zoomChar(17, 24, Charwidth, Font_Heig, (uint8_t *)&DrawProp[ActiveLayer].pFont->table[Ascii * DrawProp[ActiveLayer].pFont->Height * ((DrawProp[ActiveLayer].pFont->Width + 7) / 8)], psr, 0);
      // 显示单个字符
      LCD_DrawChar_Ex(refcolumn, y, Charwidth, Font_Heig, (uint8_t *)&zoomBuff, DrawModel);
      refcolumn += Charwidth;
      ptr++;
